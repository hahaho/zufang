...
public class BsdiffinfoService {
    ...
    public void bsdiffUpload(BsdiffVo bsdiffVo, BsdiffInfoEntity bsdiffInfoEntity) throws IOException {
		...
		if(new File(directory.getParent()).listFiles(new FileFilter() {
			...
			if(count == 1){
				//patch生成的包对应的目录
				File directoryPatch = new File(rootPath+nfsBsdiffPath+PATCHPATH+"/"+bsdiffVo.getLineId()+"/"+bsdiffVo.getBsdiffVer()+"/");
				//如果目录不存在创建目录
				if(!directoryPatch.exists()){
	...
	private void mergeFile(String zipPath, BsdiffInfoEntity bsdiffInfoEntity) throws IOException {
		//存储文件清单
		List<FileEntitis> list = Lists.newArrayList();
		//用来生成序列流的集合
		...
		try{
			...
			//记录文件清单列表
			File f = new File(descDir,files[0].getName().split("\\.")[0]+".properties");
