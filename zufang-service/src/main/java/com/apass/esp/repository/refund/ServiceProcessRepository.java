 ...
 *
 * @author chenbo
 * @version $Id: OrderRefundRepository.java, v 0.1 2017年1月3日 下午15:39:15 chenbo Exp $
 */
@MyBatisRepository
public class ServiceProcessRepository extends BaseMybatisRepository<ServiceProcessEntity, Long> {
    ...
    public List<ServiceProcessEntity> queryServiceProcessDetailInfoByParam(Map<String, String> map)
            throws BusinessException {
        try {
            List<ServiceProcessEntity> refundList = getSqlSession().selectList(
                    getSQL("queryServiceProcessDetailInfoByParam"), map);
            return refundList;
        } catch (Exception e) {
     ...
     * @throws BusinessException
     */
    public List<ServiceProcessEntity> queryServiceProcessInfoByParam(Map<String, String> map)
            throws BusinessException {
        try {
    ...
    public List<ServiceProcessEntity> updateServiceProcessDetailInfoByParam(Map<String, String> map)
        ...
        try {
            List<ServiceProcessEntity> refundList = getSqlSession().selectList(
                    getSQL("updateServiceProcessDetailInfoByParam"), map);
            return refundList;
        } catch (Exception e) {
            throw new BusinessException("查询", e);
        }
