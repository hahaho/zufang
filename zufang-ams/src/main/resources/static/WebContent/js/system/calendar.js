...
function drawCld(SY, SM) {
    ...
    hDays=[]; //将选中的工作日清空
    var startday=cld.firstWeek-1;
    var time=SY+"-"+ (SM+1)
    var param={};
    param['time']=time;
    //获取数据库中的休假日期
	$.ajax({
		url : ctx + '/application/gfb/nonworkday/queryWorkdays',
		type : "post",
		data : param,
		dataType : "json",
		success : function(data) {
			...
			var holids = data.split(",");
			setHoliday(holids,startday);
		}
	});
...
var holiDays = [];
//在页面上设置休假日期为黄色
function setHoliday(holids,startday){
	for(i=0;i<holids.length;i++){
		var nonworkday=holids[i];
		var workdays=nonworkday.split("-");
		var SY= workdays[0];
		var SM= workdays[1];
		var SD= workdays[2];
		var BeginTiem=workdays[3];
		var EndTime=workdays[4];
		var WorkType=workdays[5];
		if(WorkType=="1"){
			var sd=parseInt(SD)+parseInt(startday);
		    $("#GD" + sd).addClass("selday");
		    var dayJson = "";
		    dayJson = SY+"-"+SM+"-"+SD;
		    hDays.push(dayJson);
		    holiDays.push(nonworkday);
		}
...
//    var dayJson = "";
    ...
    if(yue.length==1){
     ...
     if(day.length==1){
    	 dayJson=nian+"-"+yue3+"-"+"0"+day;
    	 var time =nian+"-"+yue3+"-"+"0"+day;
     }else{
    	 dayJson=nian+"-"+yue3+"-"+day;
     }
     var isfalg=$.inArray(dayJson, hDays);  //判断工作日是否存在
     thisObj.attributes["on"].value='0';
     var showBeginTime="";
     var showEndTime="";
     for(i=0;i<holiDays.length;i++){
    	 var hd=holiDays[i].substring(0,10);
    	 if(hd==dayJson){
    		 showBeginTime=holiDays[i].substring(11,13);
    		 showEndTime=holiDays[i].substring(14,16);
    ...
    if(onoff == '0'){
        if(isfalg == "-1"){
    		$('#setBeginAndEndTime').window({
	            minimizable:false,
	            maximizable:false,
	            collapsible:false,
	            modal:true  
			});
    		$("#workBeginTime").combobox('setValue',"08");
        	$("#workEndTime").combobox('setValue',"18");
        	$('#setBeginAndEndTime').window('open');
        }
    }else{
    	$('#showBeginAndEndTime').window({
            minimizable:false,
            maximizable:false,
            collapsible:false,
            modal:true  
		});
    	$("#showBeginTime").combobox('setValue',showBeginTime);
    	$("#showEndTime").combobox('setValue',showEndTime);
    	$('#showBeginAndEndTime').window('open');
    	beforeEditworkday=dayJson+"-"+showBeginTime+"-"+showEndTime+"-"+"1";
    }
...
$(function() {
	$('#setBeginAndEndTime').window('close');//弹出层关闭
	$('#showBeginAndEndTime').window('close');
    initRiliIndex();
    clear();
    $("#nian").html(tY);
    $("#yue").html(tM + 1);
  ...
  //设置小时弹出框 取消按钮
    $("#cancelAddEdit").click(function(){
       $('#setBeginAndEndTime').window('close');
       dayJson = "";
    	});

    //同意
    $("#agreeAddEdit").click(function(){
    	var param={};
        var workBeginTime = $("#workBeginTime").combobox('getValue');
        var workEndTime = $("#workEndTime").combobox('getValue');
        if(workBeginTime>workEndTime){
        	alert("工作开始时间不能小于工作结束时间");
        }
        param['dayJson'] =dayJson;
        param['workBeginTime'] =workBeginTime;
        param['workEndTime'] =workEndTime;
        var addwoekday=dayJson+"-"+workBeginTime+"-"+workEndTime+"-"+"1";
        //添加休假日期
    	$.ajax({
    		url : ctx + '/application/gfb/nonworkday/addHolidays',
    		type : "post",
    		data : param,
    		dataType : "json",
    		success : function(data) {
    			if(data.status==("1")){
    				alert("操作成功");
    				$('#setBeginAndEndTime').window('close');
    			    thisObj2.setAttribute("class", "selday");
    				hDays.push(dayJson);
    				holiDays.push(addwoekday);
    			}else{
    				alert("操作失败,请刷新重试！");
    				$('#setBeginAndEndTime').window('close');
    			}
    ...
    $("#agree").click(function(){
        var workBeginTime = $("#showBeginTime").combobox('getValue');
        var workEndTime = $("#showEndTime").combobox('getValue');
        var addwoekday=dayJson+"-"+workBeginTime+"-"+workEndTime+"-"+"1";
    	var param={};
        param['dayJson'] =dayJson;
        //添加休假日期
    	$.ajax({
    		url : ctx + '/application/gfb/nonworkday/updateHolidays',
    		type : "post",
    		data : param,
    		dataType : "json",
    		success : function(data) {
    			if(data.status==("1")){
    				alert("操作成功");
    				 $('#showBeginAndEndTime').window('close');
    				 thisObj2.setAttribute("class", "");
    			     delArry(hDays,dayJson);
    			     delArry(holiDays,addwoekday);
    			     if(nian2==tY && yue2==(tM+1) && day2==tD){
    		        	thisObj2.setAttribute("class", "jinri");
    		         }
    			}else{
    				alert("操作失败,请刷新重试！");
    				 $('#showBeginAndEndTime').window('close');
    	...
    	$.ajax({
    		...
    		success : function(data) {
    			if(data.status==("1")){
    				alert("操作成功");
    				 $('#showBeginAndEndTime').window('close');
    			     delArry(holiDays,beforeEditworkday);
    				 holiDays.push(addwoekday);
    			}else{
    				alert("操作失败,请刷新重试！");
    				 $('#showBeginAndEndTime').window('close');
